...
@section('content')
    <main id="main" class="main">
      ...
      <style>
        ...
        /* Card color variants */
        .stat-card.jadwal::before { background: linear-gradient(90deg, #d97706, #f59e0b); }
        .stat-card.jadwal .stat-icon { background: rgba(217,119,6,0.1); color: #d97706; }
        .stat-card.jadwal .stat-number { color: #d97706; }
        .stat-card.jadwal .stat-arrow { background: rgba(217,119,6,0.1); color: #d97706; }
      ...
      <div class="dashboard-welcome">
          <i class="bi bi-people-fill welcome-icon"></i>
          <div class="welcome-date">
              <i class="bi bi-calendar3"></i>
              {{ \Carbon\Carbon::now()->translatedFormat('l, d F Y') }}
          </div>
          <h4><i class="bi bi-hand-thumbs-up me-2"></i>Selamat Datang, {{ Auth::user()->username }}!</h4>
          <p>Anda telah masuk sebagai <strong>{{ Auth::user()->role }}</strong>. Tetap semangat belajar dan kerjakan tugas-tugas dari guru dengan baik!</p>
      </div>

      {{-- Statistic Cards --}}
      <div class="stat-cards-grid">
          {{-- Jadwal --}}
          <a href="{{ route('siswa.jadwal-pelajaran') }}" class="stat-card jadwal">
              <div class="stat-icon">
                  <i class="bi bi-calendar"></i>
              </div>
              <div class="stat-number">{{ $jadwalCount ?? 0 }}</div>
              <div class="stat-label">Jadwal Pelajaran</div>
              <div class="stat-arrow">
                  <i class="bi bi-arrow-right"></i>
              </div>
              <i class="bi bi-calendar stat-bg-icon"></i>
          </a>
          ...
          {{-- Rapor --}}
          <a href="{{ route('siswa.rapor.index') }}" class="stat-card rapor">
              <div class="stat-icon">
                  <i class="bi bi-journal-text"></i>
              </div>
              <div class="stat-number">{{ $raporCount ?? 0 }}</div>
              <div class="stat-label">Data Rapor</div>
              <div class="stat-arrow">
                  <i class="bi bi-arrow-right"></i>
              </div>
              <i class="bi bi-journal-text stat-bg-icon"></i>
          </a>

          {{-- Tugas --}}
          <a href="{{ route('siswa-tugas.index') }}" class="stat-card tugas">
              <div class="stat-icon">
                  <i class="bi bi-bar-chart"></i>
              </div>
              <div class="stat-number">{{ $tugasCount ?? 0 }}</div>
              <div class="stat-label">Tugas Siswa</div>
              <div class="stat-arrow">
                  <i class="bi bi-arrow-right"></i>
              </div>
              <i class="bi bi-bar-chart stat-bg-icon"></i>
          </a>
      </div>

      {{-- Quick Links --}}
      <h5 class="quick-links-title"><i class="bi bi-lightning-fill"></i> Akses Cepat Siswa</h5>
      <div class="quick-links-grid">
          <a href="{{ route('siswa.jadwal-pelajaran') }}" class="quick-link-item">
              <i class="bi bi-calendar"></i> Jadwal Pelajaran
          </a>
          <a href="{{ route('materi-pembelajaran.index') }}" class="quick-link-item">
              <i class="bi bi-book"></i> Materi Pelajaran
          </a>
          <a href="{{ route('siswa.rapor.index') }}" class="quick-link-item">
              <i class="bi bi-journal-text"></i> Lihat Rapor
          </a>
          <a href="{{ route('siswa-tugas.index') }}" class="quick-link-item">
              <i class="bi bi-bar-chart"></i> Tugas Saya
          </a>
      </div>
